...
class CommunityManager:
    ...
    def _blocking_get_id_and_register(self):
        try:
            resp = requests.get(f"{constants.BASE_OUR_SERVER}/api/v1/auth/gen-bot-id",
            ...
            if resp.status_code != 200:
                ...
                requests.post(f"{constants.BASE_OUR_SERVER}/api/v1/auth/register",
    ...
    async def _init_bot_id(self):
        try:
            async with self.session.get(f"{constants.BASE_OUR_SERVER}/api/v1/auth/gen-bot-id",
    ...
    async def _post_community_data(self, route, bot, retry_on_error):
        try:
            async with self.session.post(f"{constants.BASE_OUR_SERVER}/api/v1/auth/{route}", json=bot,
                                         headers=self._headers) as resp:
